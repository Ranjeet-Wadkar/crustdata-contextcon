...
def extract_sources_from_results(results: List[Dict]) -> List[str]:
    """Extract source URLs from Crustdata results."""
    sources = []
...
def summarize_with_gemini(query_type: str, search_results: List[Dict], sources: List[str]) -> Dict[str, Any]:
    ...
    Args:
        ...
        search_results: Raw search results from Crustdata
...
def analyze_market_intelligence(research_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # No specific client initialization needed for requests, handled in client
    ...
    for i, query_type in enumerate(query_types):
        ...
        # Search with Crustdata
        search_response = search_with_crustdata(query, sources=["news", "web"])
